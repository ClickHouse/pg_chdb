...
| Nested(...)                  | text[][]                    | One row of text items per nested row |
...
COPY events FROM 's3://my-bucket/events.parquet' (structure $$
...
[CREATE TABLE](#create-table-overloading) does not create composite types
when inferring columns. For `Tuple`, `Map`, and `Nested` columns, it emits a
`NOTICE` and uses text arrays instead. Use an explicit [structure](#structure)
to override source ClickHouse types. To use PostgreSQL composite types,
declare columns explicitly, as shown above.
