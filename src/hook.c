/*
 * Claiming the COPY commands that name a URL, which Postgres has no handler
 * for. Applies the checks DoCopy() makes before handing the copy to copy.c.
 */
...
contextualize_options(chdbCopyContext* ctx, List* options) {
    ...
    ctx->max_memory    = (int64_t)chdb_max_memory * 1024 * 1024;
